...
int get_student(int fd, int id, student_t *s)
{
    ...
    // TODO: if statement for the lseek function result.
    // Solution: copied and pasted from add_student();
    if (lseek_result < 0){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    read_result = read(fd, s, STUDENT_RECORD_SIZE);

    /*
        If the result of the read() function is a negative number, I have the program 
        provide an error message, as something went awry during the read process. 
    */ 
    if (read_result < 0){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    /* 
        In this scenario, if the read result is 0 (namely that the offset is located at the end of
        the file) or the read result is equal to an EMPTY_STUDENT_RECORD, the SRCH_NOT_FOUND constant
        is returned.  
    */
    if (read_result == 0 || memcmp(s, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    /*  
        Using the get_student function, this if statement provides an error message if a record
        already exists in the location where the user attempted to add a new student. 
    */
    search_result = get_student(fd, id, &mystudent);
    ...
    /*  
        Error message for an unsuccessful seek() function. 
    */
    if(lseek_result < 0){
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    /*  
        Writing the user-provided information to the appropriate offset in the file. 
    */
    bytes_written = write(fd, &mystudent, STUDENT_RECORD_SIZE);

    /*  
        An if-statement for if the write is unsuccessful.
    */
    if (bytes_written < 0){
...
int del_student(int fd, int id)
{
    ...
    /*  
        Another use of the get_student() function to locate the appropriate location in the file. 
    */
    search_result = get_student(fd, id, &mystudent);
    ...
    /*  
        The overwriting of an existing record with 0s via the EMPTY_STUDENT_RECORD constant. 
    */
...
int count_db_records(int fd)
{
    ...
    /*  
        This while-loop reads each record until it reaches the end of the file (and the result of the 
        function is 0).
    */
    while ((bytes_read = read(fd, s, STUDENT_RECORD_SIZE)) != 0){
        ...
        /*  
            If the read result is NOT an empty record, the counter is incremented. 
        */
        else if ((result = memcmp(s, &EMPTY_STUDENT_RECORD, sizeof(student_t))) != 0){
            counter++;
        }
    }    

    /*  
        If the counter is never incremented, the file is empty. 
    */
    if (counter == 0){
        printf(M_DB_EMPTY);
        return counter;
    ...
    /*  
        Freeing the bufer we allocated earlier in the function. 
    */
    free (s);
...
int print_db(int fd)
{
    ...
    /*  
        Similar to the count_db_records() function above, this function reads each record -- 
        and in addition to reading the records, it also prints out their contents. 
    */
    while ((bytes_read = read(fd, student, STUDENT_RECORD_SIZE)) != 0){
        ...
        else if ((result = memcmp(student, &EMPTY_STUDENT_RECORD, sizeof(student_t))) != 0){

            counter++;
            
            /*  
                For the first record found, we print these headings. 
            */
            if (counter == 1){
            ...
            /*  
                For all records found, we print out their contents. 
            */
            printf(STUDENT_PRINT_FMT_STRING, student->id, student->fname, student->lname,
    ...
    /*  
        If the file is empty, we print that message to the console. 
    */
    if (counter == 0){
        printf(M_DB_EMPTY);
        return counter;
...
void print_student(student_t *s)
{
    ...
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST NAME", "GPA");
    printf(STUDENT_PRINT_FMT_STRING, s->id, s->fname, s->lname,
                (calculated_gpa_from_student = (s->gpa / 100.0)));
