...
{  
    ...
    char *send_buffer = malloc(RDSH_COMM_BUFF_SZ);
    char *recv_buffer = malloc(RDSH_COMM_BUFF_SZ);
    int ret;
    int socket;

    socket = start_client(address, port);
    if (socket == ERR_RDSH_CLIENT){
        printf(RCMD_MSG_CLIENT_EXITED); 
        client_cleanup(socket, send_buffer, recv_buffer, ERR_RDSH_CLIENT);
    ...
    while(1){
        ...
        /*  
          Setting the buffers blank -- all zeroes.
        */
        memset(send_buffer, 0, RDSH_COMM_BUFF_SZ);
        memset(recv_buffer, 0, RDSH_COMM_BUFF_SZ);

        if (fgets(send_buffer, ARG_MAX, stdin) == NULL){
        printf("\n");
        break;
        }
        //remove the trailing \n from cmd_buff
        ...
        int send_len = strlen(send_buffer) + 1;

        int bytes_sent = send(socket, send_buffer, send_len, 0);
        if (bytes_sent < 0){
            printf(CMD_ERR_RDSH_COMM);
            client_cleanup(socket, send_buffer, recv_buffer, ERR_RDSH_COMMUNICATION);
            return ERR_RDSH_COMMUNICATION;
        }

        while ((ret = recv(socket, recv_buffer, sizeof(recv_buffer), 0)) > 0){


            printf("%.*s", (int)ret, recv_buffer);


            if (recv_buffer[ret - 1] == RDSH_EOF_CHAR){

                /*  
                  This is the functionality I put in place to handle the exit command. When the client sends "exit" to 
                  the server, the server sends back only the EOR_CHAR -- and if that's the only thing in the buffer,
                  "OK" is returned to the main function to exit out of the client.  
                */
                if ((strcmp(recv_buffer, &RDSH_EOF_CHAR) == 0)){
                    client_cleanup(socket, send_buffer, recv_buffer, OK);
                    return OK;
                }
                break;
                }
...
int start_client(char *server_ip, int port){
    ...
    /*  
      This implementation is largely taken from the demo code for sockets. 
    */
